...
import type { AppConfig, KnowledgeGraphConfig, AbilityConfig, AgenticConfig, AgenticToolConfig, Usage, AgentLoopEvent } from '../types'
...
declare global {
  interface Window {
    electronAPI?: {
      ...
      // 能力文件相关（Markdown 格式，存储在 abilities 目录）
      saveAbilityFile: (name: string, content: string) => Promise<{ success: boolean; error?: string }>
      loadAbilityFile: (name: string) => Promise<{ success: boolean; content: string | null; mtime: string | null; error?: string }>
      deleteAbilityFile: (name: string) => Promise<{ success: boolean; error?: string }>
      loadAllAbilityFiles: () => Promise<{ success: boolean; files?: string[]; error?: string }>
...
// ===== 命令文件存储方法 =====
const COMMAND_FILES_KEY = 'flow-editor-command-files'
...
// 保存命令文件列表（每个命令保存为独立的 Markdown 文件）
export const saveCommandFilesToLocal = async (commands: any[]): Promise<boolean> => {
  if (!isElectron()) {
    // 浏览器环境：使用 localStorage 存储
    try {
      localStorage.setItem(COMMAND_FILES_KEY, JSON.stringify(commands))
      return true
    } catch (error) {
      console.error('保存命令文件到 localStorage 失败:', error)
      return false
    }
  }

  // Electron 环境：保存为 Markdown 文件到 commands 目录
  try {
    // 获取现有文件列表，用于删除不再需要的文件
    const existingResult = await window.electronAPI!.loadAllCommandFiles()
    const existingFiles = existingResult.success ? existingResult.files || [] : []
    ...
    for (const fileName of existingFiles) {
      ...
      if (!currentNames.includes(commandName)) {
        await window.electronAPI!.deleteCommandFile(commandName)
      }
    }
    ...
    for (const command of commands) {
      ...
      const mdContent = generateCommandMarkdown(metadata, command.content || '')

      const saveResult = await window.electronAPI!.saveCommandFile(command.name, mdContent)
      if (!saveResult.success) {
        console.error(`保存命令文件 ${command.name} 失败:`, saveResult.error)
      }
    }
...
// 加载命令文件列表
export const loadCommandFilesFromLocal = async (): Promise<any[]> => {
  if (!isElectron()) {
    // 浏览器环境
    try {
      const data = localStorage.getItem(COMMAND_FILES_KEY)
      if (!data) return []
      return JSON.parse(data)
    } catch (error) {
      console.error('从 localStorage 加载命令文件失败:', error)
      return []
    }
  }

  // Electron环境：从 commands 目录的 Markdown 文件加载
  try {
    const result = await window.electronAPI!.loadAllCommandFiles()
    ...
    if (result.success && result.files) {
      ...
      for (const fileName of result.files) {
        const commandName = fileName.replace(/\.md$/, '')
        const contentResult = await window.electronAPI!.loadCommandFile(commandName)
        if (contentResult.success && contentResult.content) {
          const { metadata, body } = parseCommandFrontmatter(contentResult.content)
...
// 删除单个命令文件
export const deleteCommandFileFromLocal = async (name: string): Promise<boolean> => {
  if (!isElectron()) {
    // 浏览器环境：由 store 处理
    return true
  }
  ...
  // Electron 环境
  try {
    const result = await window.electronAPI!.deleteCommandFile(name)
    return result.success
  } catch (error) {
...
// ===== 能力文件存储方法 =====
const ABILITY_FILES_KEY = 'flow-editor-ability-files'

// 解析能力 frontmatter（包含 name, description）
export const parseAbilityFrontmatter = (content: string): { metadata: Record<string, any>; body: string } => {
  const frontmatterRegex = /^---\n([\s\S]*?)\n---\n([\s\S]*)$/
  const match = content.match(frontmatterRegex)
...
// 保存能力文件列表（每个能力保存为独立的 Markdown 文件）
export const saveAbilityFilesToLocal = async (abilities: any[]): Promise<boolean> => {
  if (!isElectron()) {
    // 浏览器环境：使用 localStorage 存储
    try {
      localStorage.setItem(ABILITY_FILES_KEY, JSON.stringify(abilities))
      return true
    } catch (error) {
      console.error('保存能力文件到 localStorage 失败:', error)
      return false
    }
  }

  // Electron 环境：保存为 Markdown 文件到 abilities 目录
  try {
    // 获取现有文件列表，用于删除不再需要的文件
    const existingResult = await window.electronAPI!.loadAllAbilityFiles()
    const existingFiles = existingResult.success ? existingResult.files || [] : []
    const currentNames = abilities.map(a => a.name)

    // 删除不再需要的文件
    for (const fileName of existingFiles) {
      const abilityName = fileName.replace(/\.md$/, '')
      if (!currentNames.includes(abilityName)) {
        await window.electronAPI!.deleteAbilityFile(abilityName)
      }
    }
    ...
    for (const ability of abilities) {
      ...
      const mdContent = generateAbilityMarkdown(metadata, ability.content || '')

      const saveResult = await window.electronAPI!.saveAbilityFile(ability.name, mdContent)
      if (!saveResult.success) {
        console.error(`保存能力文件 ${ability.name} 失败:`, saveResult.error)
      }
    }
...
// 加载能力文件列表
export const loadAbilityFilesFromLocal = async (): Promise<any[]> => {
  if (!isElectron()) {
    // 浏览器环境
    try {
      const data = localStorage.getItem(ABILITY_FILES_KEY)
      if (!data) return []
      return JSON.parse(data)
    } catch (error) {
      console.error('从 localStorage 加载能力文件失败:', error)
      return []
    }
  }

  // Electron环境：从 abilities 目录的 Markdown 文件加载
  try {
    const result = await window.electronAPI!.loadAllAbilityFiles()
    ...
    if (result.success && result.files) {
      ...
      for (const fileName of result.files) {
        const abilityName = fileName.replace(/\.md$/, '')
        const contentResult = await window.electronAPI!.loadAbilityFile(abilityName)
        if (contentResult.success && contentResult.content) {
          const { metadata, body } = parseAbilityFrontmatter(contentResult.content)

          abilities.push({
            id: `ability-${Date.now()}-${Math.random().toString(36).slice(2)}`,
            name: metadata.name || abilityName,
            type: 'ability',
            description: metadata.description || '',
            content: body,
            createdAt: contentResult.mtime || new Date().toISOString(),
...
// 删除单个能力文件
export const deleteAbilityFileFromLocal = async (name: string): Promise<boolean> => {
  if (!isElectron()) {
    // 浏览器环境：由 store 处理
    return true
  }
  ...
  // Electron 环境
  try {
    const result = await window.electronAPI!.deleteAbilityFile(name)
    return result.success
  } catch (error) {
...
const DEFAULT_APP_CONFIG: AppConfig = {
  ...
  sidebarNavOrder: ['agents', 'commands', 'abilities', 'knowledges', 'workflows', 'nodes', 'resources'],
}
...
const DEFAULT_ABILITY_PROMPT_TEMPLATE = `## 角色
- 你是一个专业的AI能力设计助手。请根据用户提供的描述，详细理解用户的需求，生成高质量的能力内容。
...
## 注意事项
- 输出的能力内容必须详细、具体，不可泛泛而谈
- 内容结构要清晰，使用 Markdown 格式
- 包含可衡量/量化的指标和验收维度
- 提供具体的使用场景和执行步骤

## 输出要求
请直接输出能力的详细内容（Markdown格式），不需要包含名称和描述字段。输出内容应包括：
- 能力说明
- 使用场景
- 执行步骤
- 注意事项
...
// 默认的能力优化提示词模板
const DEFAULT_ABILITY_OPTIMIZE_PROMPT_TEMPLATE = `你是一个专业的AI能力优化助手。请根据用户提供的优化目标，对现有的能力内容进行优化改进。
 ...
 * 保存在能力配置
 */
export const saveAbilityConfig = async (config: AbilityConfig): Promise<void> => {
  try {
    localStorage.setItem('ability-config', JSON.stringify(config))
  } catch (error) {
    console.error('保存能力配置失败:', error)
  }
}
 ...
 * 加载能力配置
 */
export const loadAbilityConfig = async (): Promise<AbilityConfig> => {
  try {
    const stored = localStorage.getItem('ability-config')
    if (!stored) {
      // 返回默认配置
      return {
        promptTemplate: DEFAULT_ABILITY_PROMPT_TEMPLATE,
        optimizePromptTemplate: DEFAULT_ABILITY_OPTIMIZE_PROMPT_TEMPLATE,
        updatedAt: new Date().toISOString()
      }
    }
    const parsed = JSON.parse(stored)
    // 兼容旧配置，如果没有 optimizePromptTemplate 则添加默认值
    return {
      promptTemplate: parsed.promptTemplate || DEFAULT_ABILITY_PROMPT_TEMPLATE,
      optimizePromptTemplate: parsed.optimizePromptTemplate || DEFAULT_ABILITY_OPTIMIZE_PROMPT_TEMPLATE,
 ...
 * 获取默认能力提示词模板
 */
export const getDefaultAbilityPromptTemplate = (): string => {
  return DEFAULT_ABILITY_PROMPT_TEMPLATE
}

/**
 * 获取默认能力优化提示词模板
 */
export const getDefaultAbilityOptimizePromptTemplate = (): string => {
  return DEFAULT_ABILITY_OPTIMIZE_PROMPT_TEMPLATE
}
...
// ===== 能力模块模板文件存储方法（持久化到本地文件）=====

/**
 * 保存能力模板到本地文件
 * @param templateType 模板类型: 'smart-create' | 'optimize' | 'agentic-create' | 'agentic-optimize'
 * @param content 模板内容
 */
export const saveAbilityTemplateFile = async (templateType: 'llm-create' | 'llm-optimize' | 'agentic-create' | 'agentic-optimize', content: string): Promise<void> => {
  if (!isElectron()) {
 ...
 */
export const loadAbilityTemplateFile = async (templateType: 'llm-create' | 'llm-optimize' | 'agentic-create' | 'agentic-optimize'): Promise<string> => {
  // 获取对应类型的默认模板
