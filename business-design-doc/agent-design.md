...
### 接入说明
智能体模块的创建弹窗已接入 Agentic 能力，用户可选择"手动创建"或"Agentic 创建"两种方式。

### 创建流程
1. 打开创建弹窗时展示创建方式选择界面
2. 选择"Agentic 创建"，输入智能体角色描述
3. Agent Loop 自动执行：查看已有智能体文档 → 生成角色指令内容 → 回填表单
4. 用户可对生成结果二次编辑后保存

### 模板配置
Agentic 创建/优化提示词模板可在 **设置 → 智能体** 页面中编辑、预览、保存和重置。
