...
import { BookOpen, Network, FolderTree, Settings, ShieldCheck } from 'lucide-react'
...
export const KnowledgeSidebar: FC = () => {
  const { knowledgeSubPage, setKnowledgeSubPage } = useAppStore()

  return (
    <nav className="w-44 h-full flex flex-col pt-4 pb-4">
      {subNavItems.map((item) => {
        const isActive = knowledgeSubPage === item.id
        const Icon = item.icon

        return (
          <div key={item.id} className="px-2 mb-1">
            <motion.button
              onClick={() => setKnowledgeSubPage(item.id)}
              whileTap={{ scale: 0.98 }}
              className={`
