...
import { Type, Layers, MessageSquare, Eye, Edit3, PenLine, Bot, Loader2, Wand2 } from 'lucide-react'
import { Modal, Input, Textarea, Button, ConfirmModal, MarkdownEditor, MarkdownRenderer } from '../ui'
import { useToastStore } from '../../stores/toastStore'
import { useSettingsStore } from '../../stores/settingsStore'
import { useProjectStore } from '../../stores/projectStore'
import { isElectron, loadNodeTemplateFile } from '../../utils/storage'
...
export const NodeModal: FC<NodeModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (mode === 'edit' && initialData) {
        ...
        setNodeType(initialData.type as NodeType)
  ...
  const handleAgenticGenerate = async () => {
    ...
    const promptTemplate = await loadNodeTemplateFile('agentic-create')

    let task: string
    if (promptTemplate.includes('{{userDescription}}')) {
      task = promptTemplate.replace(/\{\{userDescription\}\}/g, userDescription)
    } else {
      console.warn('[Agentic Node Create] 模板中未找到 {{userDescription}} 占位符，将自动追加用户描述')
      task = `${promptTemplate}\n\n## 用户需求\n${userDescription}`
    }
    ...
    console.log('[Agentic Node Create] 开始执行 Agent Loop')
    console.log('[Agentic Node Create] 任务:', userDescription)
    console.log('[Agentic Node Create] 提供商:', selectedProvider.name)
    console.log('[Agentic Node Create] 模型:', selectedModel)
    console.log('[Agentic Node Create] 工具:', enabledTools.map(t => t.type).join(', '))
    console.log('='.repeat(60))
    ...
    try {
      ...
      console.error('[Agentic Node Create] 执行失败:', error)
    }
  }
  ...
  const handleSubmit = () => {
    ...
    if (mode === 'create' && existingNames.includes(name.trim())) {
      addToast('节点名称已存在，请使用其他名称', 'warning')
      return
    }

    if (!content.trim()) {
      setInvalidFields(new Set(['content']))
      addToast('请输入节点内容', 'warning')
      setTimeout(() => setInvalidFields(new Set()), 3000)
      return
    }
  ...
  const renderAgenticCreate = () => (
    <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
      <div>
        ...
        <Textarea
          placeholder="例如：帮我创建一个文档解析节点，可以从文件中提取结构化内容...&#10;&#10;Agentic 模式会：&#10;1. 首先查看 .claude/nodes/ 目录下已有的节点文档&#10;2. 参考已有文档的格式和风格&#10;3. 根据你的描述创建新的节点文档"
  ...
  const renderManualEdit = () => (
    <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
      ...
      <div>
        ...
        />
        {mode === 'edit' && (
          <p className="mt-1 text-xs text-macos-text-tertiary">编辑模式下节点名称不可修改</p>
        )}
      </div>
      ...
      <div>
        ...
        {viewMode === 'edit' && (
          <MarkdownEditor
            placeholder="在此输入节点内容，支持 Markdown 格式...&#10;&#10;例如：&#10;# 节点说明&#10;- 这个节点的核心功能是...&#10;&#10;提示：输入@和%可引用其他业务内容"
            value={content}
            onChange={(e) => {
