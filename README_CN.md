...
基于 `Electron` + `React` + `TypeScript` 构建的桌面应用，以 Markdown 文件为核心数据载体，完全本地存储，为编码智能体（Coding-Agent）提供智能体、技能、知识、节点、资源文件、工作流等资产的统一管理与可视化编排。
...
<p align="center">
  <img src="./assets/preview.png" alt="Ocean 预览" width="800" />
</p>
...
编码智能体（Coding-Agent）是强大的 AI 工具，但其资产（智能体、技能、知识、节点、资源文件、工作流）的管理分散在各个目录和文本文件中。Ocean 是专为编码智能体打造的统一可视化管理平台，将这些分散的 Markdown 文件转化为一个连贯、可管理的系统。

- **引用而非复制** -- 资产之间通过 `@` 引用和 `%` WikiLink 建立关联，而非复制内容。单一数据源意味着修改一个共享资产或知识条目后，所有引用处自动同步，无需重复维护多份副本。
- **知识图谱化** -- 每条知识以独立 Markdown 文件存储，通过带关系标签的 WikiLink 互联。整个知识网络渲染为可交互的力导向图谱，点击节点即可查看详情，还可调节向心力、节点距离等物理参数。
- **零数据锁定** -- 所有数据以标准 Markdown 文件存储在项目的资产目录中（`.claude/`、`.pi/`、`.knowledges/`、`.nodes/`、`.resources/`、`.workflows/`）。你可以用任何文本编辑器直接编辑，也可以使用 Ocean 的可视化界面管理。
...
Ocean 管理 6 种编码智能体资产，每种资产均支持完整的增删改查、Markdown 预览和引用链接：

智能体和技能从可切换的资产来源加载（设置 > 资产来源）；知识、节点、资源文件和工作流存储在项目级共享目录中。

| 模块 | 存储位置 | 说明 |
|------|---------|------|
| 智能体 (Agents) | `.claude/agents/` 或 `.pi/agents/` | 定义 AI 智能体配置，支持模型选择、角色指令、图标自定义 |
| 技能 (Skills) | `.claude/skills/` 或 `.pi/skills/` | 以目录结构打包复杂技能，包含脚本、参考文档和示例 |
| 知识库 (Knowledge) | `.knowledges/` | 管理业务知识，支持标签、分类、WikiLink 引用和可视化知识图谱 |
| 节点 (Nodes) | `.nodes/` | 定义可复用的工作流构建块 |
| 资源文件 (Resources) | `.resources/` | 管理参考资源文件 |
...
- **`@` 引用** -- 在 Markdown 编辑器中输入 `@` 即可引用任意资产（智能体、技能、知识条目、资源文件、节点、工作流）。引用以文件路径的形式存储，而非复制内容。当源资产更新时，所有引用处自动同步。
...
│   │   ├── node/                # 节点模块组件
│   │   ├── resource/            # 资源模块组件
│   │   ├── settings/            # 设置组件（LLM、CLI Agent、资产来源、Agentic）
│   │   ├── skill/               # 技能模块组件
│   │   ├── ui/                  # 通用 UI 组件（Modal、MarkdownEditor、MarkdownRenderer）
│   │   └── workflow/            # 工作流模块组件
│   ├── hooks/                   # 自定义 React Hooks
│   │   ├── useAgentLoop.ts      # Agent 循环执行 Hook
│   │   ├── useAgenticExecutor.tsx # Agentic 模式执行器 Hook
│   │   ├── useKnowledgeGraph.ts # 知识图谱 Hook
│   │   └── useReferenceItems.ts # 跨资产引用 Hook
│   ├── services/                # 业务逻辑服务
...
所有数据以 Markdown 文件（可选 YAML Frontmatter）形式存储在项目目录中：

```
your-project/
├── .claude/  (或 .pi/)          # 资产来源根目录（可在设置中切换）
│   ├── agents/                  # 智能体定义 (*.md)
│   └── skills/                  # 技能包（每个技能一个目录）
│       └── skill-name/
│           ├── SKILL.md
...
│           ├── references/
│           └── examples/
├── .knowledges/                 # 知识条目 (*.md) -- 跨资产来源共享
├── .nodes/                      # 节点定义 (*.md) -- 共享
├── .resources/                  # 资源文件 (*.md) -- 共享
└── .workflows/                  # 工作流定义 -- 共享
    └── workflow-name/
        ├── WORKFLOW.md          # 生成的工作流文档
        └── flow.json            # 图结构数据（节点与边）
```

智能体和技能根据当前资产来源从 `.claude/` 或 `.pi/` 加载（可在设置中切换）；知识、节点、资源文件和工作流存储在项目级共享目录中，供两个生态共用。
