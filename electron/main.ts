...
let currentProjectPath: string | null = null  // 当前项目路径

// 应用配置文件路径（存储在用户目录）
const getAppConfigPath = () => {
  const userDir = app.getPath('userData')
  return path.join(userDir, 'ocean-config.json')
}
...
// 获取工作流文件路径（详情）
const getWorkflowFilePath = (id: string) => {
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    minWidth: 900,
    minHeight: 600,
    titleBarStyle: 'hiddenInset', // macOS风格隐藏标题栏
...
ipcMain.handle('save-workflow-file', (_, name: string, content: string) => {
  try {
...
// 加载单个工作流文件内容
ipcMain.handle('load-workflow-file', (_, name: string) => {
  try {
    const workflowsDir = getWorkflowsDir()
...
// 删除单个工作流文件
ipcMain.handle('delete-workflow-file', (_, name: string) => {
  try {
    const workflowsDir = getWorkflowsDir()
    const filePath = path.join(workflowsDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
// 获取工作流文件夹路径
const getWorkflowFolderPath = (name: string) => {
  return path.join(getWorkflowsDir(), name)
}

// 获取工作流meta-data目录路径
const getWorkflowMetaDataPath = (name: string) => {
  return path.join(getWorkflowFolderPath(name), 'meta-data')
}

// 创建工作流文件夹
ipcMain.handle('create-workflow-folder', (_, name: string) => {
  try {
...
ipcMain.handle('load-workflow-md', (_, name: string) => {
  try {
...
ipcMain.handle('save-workflow-md', (_, name: string, content: string) => {
  try {
    const folderPath = getWorkflowFolderPath(name)
    // 确保文件夹存在
    if (!fs.existsSync(folderPath)) {
...
// 加载工作流的flow.json
ipcMain.handle('load-workflow-flow-json', (_, name: string) => {
  try {
...
// 保存工作流的flow.json
ipcMain.handle('save-workflow-flow-json', (_, name: string, content: string) => {
  try {
    const metaDataPath = getWorkflowMetaDataPath(name)
    // 确保meta-data文件夹存在
    if (!fs.existsSync(metaDataPath)) {
...
// 删除工作流文件夹
ipcMain.handle('delete-workflow-folder', (_, name: string) => {
  try {
    const folderPath = getWorkflowFolderPath(name)
    if (fs.existsSync(folderPath)) {
...
// 重命名工作流文件夹
ipcMain.handle('rename-workflow-folder', (_, oldName: string, newName: string) => {
  try {
...
ipcMain.handle('save-node-file', (_, name: string, content: string) => {
  try {
...
// 加载单个节点文件内容
ipcMain.handle('load-node-file', (_, name: string) => {
  try {
    const nodesDir = getNodesDir()
...
// 删除单个节点文件
ipcMain.handle('delete-node-file', (_, name: string) => {
  try {
    const nodesDir = getNodesDir()
    const filePath = path.join(nodesDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
// 获取工作流局部节点目录路径
const getWorkflowLocalNodesPath = (workflowName: string) => {
  return path.join(getWorkflowFolderPath(workflowName), 'nodes')
}

// 保存局部节点文件
ipcMain.handle('save-local-node', (_, workflowName: string, nodeName: string, content: string) => {
  try {
...
// 加载局部节点文件
ipcMain.handle('load-local-node', (_, workflowName: string, nodeName: string) => {
  try {
    const nodesDir = getWorkflowLocalNodesPath(workflowName)
...
// 加载所有局部节点文件列表
ipcMain.handle('load-all-local-nodes', (_, workflowName: string) => {
  try {
    const nodesDir = getWorkflowLocalNodesPath(workflowName)
    if (!fs.existsSync(nodesDir)) {
      return { success: true, files: [] }
...
// 删除局部节点文件
ipcMain.handle('delete-local-node', (_, workflowName: string, nodeName: string) => {
  try {
    const nodesDir = getWorkflowLocalNodesPath(workflowName)
    const filePath = path.join(nodesDir, `${nodeName}.md`)
    if (fs.existsSync(filePath)) {
...
ipcMain.handle('save-resource-file', (_, name: string, content: string) => {
  try {
...
// 加载单个资源文件内容
ipcMain.handle('load-resource-file', (_, name: string) => {
  try {
    const resourcesDir = getResourcesDir()
...
// 删除单个资源文件
ipcMain.handle('delete-resource-file', (_, name: string) => {
  try {
    const resourcesDir = getResourcesDir()
    const filePath = path.join(resourcesDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
ipcMain.handle('save-agent-file', (_, name: string, content: string) => {
  try {
...
// 加载单个智能体文件内容
ipcMain.handle('load-agent-file', (_, name: string) => {
  try {
    const agentsDir = getAgentsDir()
...
// 删除单个智能体文件
ipcMain.handle('delete-agent-file', (_, name: string) => {
  try {
    const agentsDir = getAgentsDir()
    const filePath = path.join(agentsDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
ipcMain.handle('save-ability-file', (_, name: string, content: string) => {
  try {
...
// 加载单个能力文件内容
ipcMain.handle('load-ability-file', (_, name: string) => {
  try {
    const abilitiesDir = getAbilitiesDir()
...
// 删除单个能力文件
ipcMain.handle('delete-ability-file', (_, name: string) => {
  try {
    const abilitiesDir = getAbilitiesDir()
    const filePath = path.join(abilitiesDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
ipcMain.handle('save-knowledge-file', (_, name: string, content: string) => {
  try {
...
// 加载单个知识库文件内容
ipcMain.handle('load-knowledge-file', (_, name: string) => {
  try {
    const knowledgesDir = getKnowledgesDir()
...
// 删除单个知识库文件
ipcMain.handle('delete-knowledge-file', (_, name: string) => {
  try {
    const knowledgesDir = getKnowledgesDir()
    const filePath = path.join(knowledgesDir, `${name}.md`)
    if (fs.existsSync(filePath)) {
...
// 保存应用配置
ipcMain.handle('save-app-config', (_, config: any) => {
  try {
    const configPath = getAppConfigPath()
...
// 初始化项目目录
ipcMain.handle('init-project-dir', (_, projectPath: string) => {
  try {
    // 设置项目路径
...
// 设置项目路径（用于切换项目）
ipcMain.handle('set-project-path', (_, projectPath: string) => {
  try {
    setProjectPath(projectPath)
    return {
      success: true,
...
// 保存知识图谱配置
ipcMain.handle('save-knowledge-graph-config', (_, config: any) => {
  try {
    if (!currentProjectPath) {
...
/**
 * 测试 LLM 连接 (使用原生 fetch)
 */
ipcMain.handle('test-llm-connection', async (_, provider: any) => {
  console.log('\n=== Electron 主进程: 使用原生 fetch 测试 LLM 连接 ===')
  ...
  try {
    ...
    if (provider.type === 'anthropic') {
      requestBody = {
        model: modelId,
        max_tokens: 1024,
        messages: [{ role: 'user', content: 'Hello, this is a test message. Please reply with "OK".' }],
      }
    } else {
      requestBody = {
        model: modelId,
 ...
 * 测试可执行文件路径
 */
ipcMain.handle('test-executable-path', async (_, filePath: string) => {
 ...
 * 保存 LLM 配置到文件
 */
ipcMain.handle('save-llm-config', async (_, config: any) => {
  try {
 ...
 * 获取 Agentic 配置文件路径
 */
const getAgenticConfigPath = (): string => {
  try {
 ...
 * 保存 Agentic 配置到文件
 */
ipcMain.handle('save-agentic-config', async (_, config: any) => {
  try {
 ...
 * 获取或创建工具实例
 */
function getTool(type: string, cwd: string): Tool | null {
  const cacheKey = `${type}:${cwd}`

  if (toolInstanceCache.has(cacheKey)) {
    return toolInstanceCache.get(cacheKey)!
  }
 ...
 */
ipcMain.handle('execute-agentic-tool', async (_, params: any) => {
 ...
 */
const sendAgentLoopEvent = (event: { type: string; data: Record<string, unknown> }) => {
 ...
 * 构建 LLM 可用的工具定义
 */
const buildToolsDefinition = (enabledTools: Array<{ type: string; description: string }>) => {
  const tools: Array<{
    type: 'function'
    function: {
      name: string
...
// 工具实例缓存
const toolInstanceCacheForLoop = new Map<string, Tool>()

/**
 * 执行工具调用
 */
const executeToolCallForLoop = async (toolName: string, toolArgs: Record<string, unknown>, cwd: string) => {
  try {
    // 获取或创建工具实例
    const cacheKey = `${toolName}:${cwd}`
    let tool = toolInstanceCacheForLoop.get(cacheKey)
    ...
    if (!tool) {
      ...
      if (tool) {
        toolInstanceCacheForLoop.set(cacheKey, tool)
      }
    }
 ...
 * 调用 LLM API（支持工具调用）
 */
const callLLMWithToolsForLoop = async (
  provider: { type: string; baseUrl: string; apiKey: string; defaultModel: string },
...
ipcMain.handle('run-agent-loop', async (_, config: {
  provider: { type: string; baseUrl: string; apiKey: string; defaultModel: string; name: string }
  model: string
  tools: Array<{ type: string; enabled: boolean; description: string }>
...
- 完成任务后，提供简洁的总结`
    ...
    for (let iteration = 0; iteration < maxIterations; iteration++) {
      ...
      // 调用 LLM
      const llmResponse = await callLLMWithToolsForLoop(provider, messages, toolDefinitions, model)
      ...
      if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
        ...
        const toolCallMessage = {
          ...
          tool_calls: llmResponse.toolCalls.map((tc: { id: string; name: string; arguments: unknown }) => ({
            id: tc.id,
            type: 'function',
            function: {
              name: tc.name,
        ...
        for (const toolCall of llmResponse.toolCalls) {
          ...
          // 执行工具
          const toolResult = await executeToolCallForLoop(
            toolCall.name,
 ...
 *
 * 加载能力模板文件
 * @param templateType 模板类型: 'llm-create' | 'llm-optimize' | 'agentic-create' | 'agentic-optimize'
 */
ipcMain.handle('load-ability-template-file', async (_, templateType: 'llm-create' | 'llm-optimize' | 'agentic-create' | 'agentic-optimize') => {
  try {
    const projectRoot = getProjectRoot()
    ...
    return { success: false, error: error instanceof Error ? error.message : String(error), content: null }
  }
})