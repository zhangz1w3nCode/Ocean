...
import {
  ...
  getDefaultAgentAgenticOptimizePromptTemplate,
...
export const AgentSettings: FC = () => {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      try {
        const createTpl = await loadAgentTemplateFile('agentic-create')
        const optimizeTpl = await loadAgentTemplateFile('agentic-optimize')
  ...
  const handleSave = async () => {
    try {
      ...
      // 保存 Agentic 优化模板
      await saveAgentTemplateFile('agentic-optimize', agenticOptimizeTemplate)
  ...
  const handleReset = () => {
    switch (activeTab) {
      ...
      case 'agentic-optimize':
        setAgenticOptimizeTemplate(getDefaultAgentAgenticOptimizePromptTemplate())
        break
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto">
        ...
        {activeTab === 'agentic-optimize' && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Wand2 size={18} className="text-blue-500" />
                <h3 className="text-base font-medium text-gray-800">智能体Agentic优化提示词模板</h3>
              </div>
              {/* 编辑/预览切换 */}
              <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
                <button
            ...
            <p className="text-sm text-gray-500 mb-3">
              配置 Agentic 优化智能体时使用的提示词模板。模板中使用 <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{'{{currentContent}}'}</code> 作为当前内容的占位符，<code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{'{{optimizeTarget}}'}</code> 作为优化目标的占位符。
            </p>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          {activeTab === 'agentic-optimize' && (
            <ul className="text-sm text-gray-600 space-y-1.5 list-disc list-inside">
              <li>Agentic 优化智能体时，系统会将当前内容和优化目标替换模板中的占位符</li>
              <li>Agent 会根据优化目标对现有智能体角色指令进行改进</li>
