...
export const StartNode: FC<StartNodeProps> = ({ data, selected, id }) => {
  ...
  return (
    <div
      className={`
        ...
        ${selected ? 'border-green-400 ring-4 ring-green-100' : ''}
        ${isConnecting ? 'border-blue-400 ring-4 ring-blue-100' : ''}
        ${isHovered && !selected && !isConnecting ? 'border-green-300 shadow-xl scale-[1.02]' : ''}
        ${!selected && !isConnecting && !isHovered ? 'border-gray-200' : ''}
      `}
    >
      <div className="flex items-center gap-2">
        <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isConnecting ? 'bg-blue-100' : 'bg-green-100'}`}>
          <Play size={12} className={isConnecting ? 'text-blue-600' : 'text-green-600'} />
        </div>
