...
export const AbilityCard: FC<AbilityCardProps> = ({ ability, onClick, onEdit, onDelete }) => {
  // 提取能力内容的前 100 个字符作为预览
  const contentPreview = ability.content?.replace(/[#*`]/g, '').slice(0, 100) || ''
    ...
    >
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-100">
              <Zap size={18} className="text-gray-600" strokeWidth={1.5} />
            </div>
            {/* 名称 */}
      ...
      <div className="flex-1 mx-4 mb-4 mt-0 p-4 rounded-lg bg-gray-50">
        <p className="text-xs text-gray-500 leading-relaxed line-clamp-3">
          {contentPreview || ability.description || '暂无内容'}
        </p>
      </div>
    </Card>
  )
