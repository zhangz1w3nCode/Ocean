...
import {
  getDefaultNodeAgenticCreatePromptTemplate,
  getDefaultNodeAgenticOptimizePromptTemplate,
  loadNodeTemplateFile,
  saveNodeTemplateFile
} from '../../utils/storage'
...
export const NodeSettings: FC = () => {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      try {
        const createTpl = await loadNodeTemplateFile('agentic-create')
        const optimizeTpl = await loadNodeTemplateFile('agentic-optimize')
  ...
  const handleSave = async () => {
    try {
      await saveNodeTemplateFile('agentic-create', agenticCreateTemplate)
      await saveNodeTemplateFile('agentic-optimize', agenticOptimizeTemplate)
  ...
  const handleReset = () => {
    switch (activeTab) {
      case 'agentic-create':
        setAgenticCreateTemplate(getDefaultNodeAgenticCreatePromptTemplate())
        break
      case 'agentic-optimize':
        setAgenticOptimizeTemplate(getDefaultNodeAgenticOptimizePromptTemplate())
        break
    }
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-medium text-gray-800">节点Agentic创建提示词模板</h3>
              </div>
            ...
            <p className="text-sm text-gray-500 mb-3">
              配置 Agentic 创建节点时使用的提示词模板。模板中使用 <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{'{{userDescription}}'}</code> 作为用户输入描述的占位符。
            </p>
            {viewMode === 'edit' && (
        ...
        {activeTab === 'agentic-optimize' && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Wand2 size={18} className="text-blue-500" />
                <h3 className="text-base font-medium text-gray-800">节点Agentic优化提示词模板</h3>
              </div>
              <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
                <button type="button" onClick={() => setViewMode('edit')} className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium transition-all ${viewMode === 'edit' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  <Edit3 size={14} /> 编辑
            ...
            <p className="text-sm text-gray-500 mb-3">
              配置 Agentic 优化节点时使用的提示词模板。模板中使用 <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{'{{currentContent}}'}</code> 作为当前内容的占位符，<code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{'{{optimizeTarget}}'}</code> 作为优化目标的占位符。
            </p>
            {viewMode === 'edit' && (
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          {activeTab === 'agentic-create' && (
            <ul className="text-sm text-gray-600 space-y-1.5 list-disc list-inside">
              <li>Agentic 创建节点时，系统会将用户输入的描述替换模板中的占位符</li>
              <li>Agent 会根据提示词自主执行任务生成节点文档内容</li>
              <li>Agent 可以使用工具查看已有节点文档作为参考</li>
              <li>生成后用户可以二次编辑，确认后才会保存</li>
            </ul>
          )}
          {activeTab === 'agentic-optimize' && (
            <ul className="text-sm text-gray-600 space-y-1.5 list-disc list-inside">
              <li>Agentic 优化节点时，系统会将当前内容和优化目标替换模板中的占位符</li>
              <li>Agent 会根据优化目标对现有节点文档进行改进</li>
